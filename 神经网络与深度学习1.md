...
$$
price=w_{area} \cdot area+w_{age} \cdot age + b \tag{1.1}
$$

​	式（1.1）中的area和age都是特征，w_area和w_age成为权重，权重决定了每个特征对预测结果的影响，	b是偏置。
...
3. 将式（1.1）中的特征数扩展到d维，则可写为
   $$
   \hat{y}=w_1x_1+w_2x_3+...+w_dx_d+b\tag{1.3}
   $$
   可以写成矩阵形式
   $$
   ...
   \tag{1.4}
   $$
   式（1.4）中表示了一条训练数据中有d维特征时的预测情况，当训练数据扩展到n条时，则可写为
   $$
...
4. 损失函数
   ...
   在有了式（1.5）中的模型后，还需要一种度量模型质量的方式——损失函数
   $$
   J(\Theta)=\frac{1}{2n}\sum_{i=1}^n(y^{(i)}-h_\theta(\mathbf{x}^{(i)}))^2\tag{1.6}
   $$
   模型训练的目标就是找到一组合适的超平面参数**w**与b，使得损失函数的值最小，即
   $$
   \min_\Theta J(\Theta)\tag{1.7}
   $$
   在线性回归模型中，可以得到解析解
   $$
   \cfrac{\partial J(\Theta)}{\partial \Theta}=0\\
   \Theta=(\mathbf{X}^T\mathbf{X})^{-1}\mathbf{X}^T\mathbf{y} \tag{1.8}
   $$
...
5. 线性分类问题
   ...
   $$
   直线方程:\theta_1x_1+\theta_2x_2+\theta_0=0(可扩展为超平面方程)\\
   z=\theta_1x_1+\theta_2x_2+\theta_0(当点位于直线上方时z为正，位于下方时z为负)\\
   \hat{y}=h_\theta(\mathbf{x}^{(i)})=\cfrac{1}{1+e^{-z}}(Sigmoid函数/S函数)\tag{1.9}
   $$
   经过式（1.9）中的变换，可将“点在直线上方/下方“的问题等价为”输出y位于0.5~1之间/0~0.5之间“。同样也需要给定一个损失函数
   $$
   J(\Theta)=\cfrac{1}{2n}\sum_{i=1}^n(y^{(i)}-h_\Theta(\mathbf{x}^{(i)}))^2\tag{1.10}
   $$
   注意这里的y只能取0/1。
...
6. 梯度下降法
   ...
   用数学表达式描述：
   $$
   \theta=\theta-\cfrac{\eta}{|\mathbf{B}|}\sum_{i\in\mathbf{B}}\cfrac{\partial(y^{(i)}-h_{\theta}(\mathbf{x}^{(i)}))^2}{\part \theta}(\eta为学习率、|\mathbf{B}|为小批量中的样本数/大小)\\
